...
核心约定：Python 3.12+、Telethon 用户账号 MTProto、APScheduler、SQLAlchemy + SQLite、Typer、Pydantic、Docker；支持手机号/验证码和二维码登录；任务支持直接指令、消息等待、按钮点击链；固定每日时间或指定时区内随机到秒；失败重试、执行历史、管理员 Telegram 失败通知；同账号同聊天串行执行。
